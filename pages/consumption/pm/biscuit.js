...
import totalInnerFoil from '../../../utils/totalInnerFoil';
...
export default function PM({products}) {
  ...
  const totalInnerFoilByCarton = product.ingredients && carton ? totalInnerFoil(product,carton) : 0
          ...
          {product?.ingredients && <div className='space-y-2'>
            <div className='consumption space-y-2'>
                ...
                <div className='border shadow rounded-md'>
                    ...
                    {product?.id === 'Valencia_Orange_Family' && <p className='flex justify-between p-2 items-center'>
                        <span className='w-3/4'>Inner Foil Consumpotion By Carton</span>
                        <span className='w-1/4 text-center'> {totalInnerFoilByCarton}</span>
                    </p>}
                </div>
            </div>
          </div>}
